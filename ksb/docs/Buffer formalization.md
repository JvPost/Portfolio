...
### 2.2 Distributional Models for Upstream Spacing

The analysis in the following sections treats $\sigma_u$ as a scalar parameter and is
intentionally agnostic about the specific distribution of inter-object spacings. However,
the choice of distributional model determines the statistical structure of the arrival
stream and therefore affects what can be predicted about skip timing, accumulated
pre-acceleration area, and feedforward feasibility. Two models are used in this work.

**Truncated normal (i.i.d. model).** Each inter-object spacing $g_i^u$ is drawn
independently from a normal distribution truncated below at $g_{\min}^u$ (the minimum
physical gap set by object length):

$$g_i^u \sim \mathcal{N}(\mu_u, \sigma_u^2) \cdot \mathbf{1}[g_i^u \geq g_{\min}^u]$$

Spacings are independent across objects: knowing that object $i$ arrived late tells you
nothing about object $i+1$. The phase drift $\phi_i$ therefore accumulates as a sum of
i.i.d. increments, and the inter-skip interval $Q_s$ has a distribution that is symmetric
around its mean $Q$. This model is analytically clean and well-suited for studying the
effect of $\sigma_u$ in isolation.

**AR(1) in log-space (autocorrelated model).** Each inter-object spacing $g_i^u$ is
generated by an AR(1) process in log-space, so that spacings are positively autocorrelated
and guaranteed positive. The excess gap $e_i = g_i^u - g_{\min}^u$ follows:

$$ \log e_i = \mu_y + \phi_{\text{AR}}(\log e_{i-1} - \mu_y) + \varepsilon_i, \qquad
\varepsilon_i \sim \mathcal{N}(0, \sigma_\varepsilon^2) $$

where $\phi_{\text{AR}} \in (0,1)$ is the autocorrelation coefficient and $\mu_y$,
$\sigma_\varepsilon$ are chosen to match the target mean $\mu_u$ and marginal standard
deviation $\sigma_u$. Under this model, a late arrival is likely to be followed by
another late arrival. After a skip triggered by an anomalously large gap, the subsequent
gaps are expected to remain above average for several objects — meaning $\phi_i$ drifts
more slowly than the mean rate immediately after a skip, and the next skip interval $Q_s$
tends to be longer than $Q$. This autocorrelation structure is more physically realistic
for production lines where spacing irregularity has mechanical causes that persist across
consecutive objects.

The key structural difference between the two models is:

- Under the **i.i.d. model**, individual skip intervals $Q_s$ are independent and
  symmetrically distributed around $Q$. The accumulated pre-acceleration area $A(s)$
  and the feedforward feasibility condition depend only on the marginal distribution of
  spacings.
- Under the **AR(1) model**, skip intervals are positively autocorrelated with preceding
  gap sizes. Skips triggered by large gaps tend to be followed by longer-than-average
  intervals, and the post-skip phase drift is systematically slower. This creates
  asymmetry in the distribution of $Q_s$ conditional on the triggering gap.

In both cases the mean skip interval $\mathbb{E}[Q_s] = Q$ is invariant, and the
long-run system behaviour is identical. The distributional model only affects the
variance and correlation structure of individual skip events.
...
$$
### 2.5 The Load Ratio and Skip Mechanism
